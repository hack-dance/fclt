...
async function makeTempHome(): Promise<string> {
  const base = join(tmpdir(), "fclt-ai-state-tests");
...
describe("ai-state freshness repair", () => {
  ...
  it("rebuilds stale legacy repo-local project indexes during migration", async () => {
    ...
    const rebuilt = JSON.parse(await readFile(result.path, "utf8")) as any;
    expect(rebuilt.instructions.GLOBAL.path).toContain("GLOBAL.md");
